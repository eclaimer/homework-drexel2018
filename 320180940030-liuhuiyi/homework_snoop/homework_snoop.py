...
class Counter:
    ...
    def scatter(self,x_column='lv',y_column='bugs'):
        try:
            label_names = {'lv':'kernel sublevel stable release',
                           'bugs':'stable fix commits',
                           'hour':'hours spent'}
            filenames = {'lv':'sublevels',
                         'bugs':'fixes',
                         'hour':'hours'}
            plt.scatter(x = self.result[x_column], y = self.result[y_column])
            plt.title("development of {y} over {x}".format(y=y_column,x=x_column))
            plt.ylabel(label_names[y_column])
            plt.xlabel(label_names[x_column])
            plt.savefig("scatter_{}_{}.png".format(filenames[x_column],self.rev))
            plt.show()
            plt.clf()
        except KeyError:
            write_log('The column name does not exist')
            raise KeyError
...
if __name__ == "__main__":
    ...
    print(title)
    print(result)
    r.scatter(y_column='bugs',x_column='hour')
    r.scatter(y_column='bugs', x_column='lv')
